...
contains
    ...
    subroutine configure_monkey(themonkey, notes)
        ...
        ! we need the divisor earlier than in part a
...
! store only modulo of item
            item = modulo(item, themonkey%test_divisor)
        ...
        ! themonkey%test_divisor moved above
    ...
    subroutine calculate_monkey(activemonkeyid, monkeys)
        ...
        do while(.not. monkeys(activemonkeyid)%items%empty())
            ...
            ! adjust operation_factor to keep only the modulo
            operation_factor = modulo(operation_factor, test_divisor)

            ! perform operation based on operator, with item and operation_factor already modulo
            ...
            item = modulo(item, test_divisor)

            ! divide worry level by three only in part a
            ! item = item / 3
